...
**Dual-screen capture scope (explicit):**
- **Now (V3 core):** two-device workflow remains primary (tablet holds session/reference, phone captures).
- **Now (V3 scoped optional beta):** single-device dual-screen mode (reference playback + camera record simultaneously) is available only behind an explicit user toggle, never default-on.
- **Not V3 core:** mandatory dual-screen default on all devices.
- **Constraint:** if performance drops below target (thermal/fps/audio latency), app auto-falls back to standard capture mode.
...
### V3 Page Coverage Requirement (Design -> App)

The following Figma pages are mandatory V3 scope and must be implemented/aligned in app code even when they are not present inside the `v3-locked` canvas:
...
For release readiness, each page above must meet the same acceptance bar used for locked pages:

- Route exists and is reachable from normal navigation flow
- Core layout and interaction model matches Figma intent
- Basic data wiring is connected (no placeholder-only surface)
...
- **Now (V3 core):** timing landmarks in workbench/song-map (downbeats, 8-count boundaries, section boundaries) for faster rehearsal navigation.
- **Now (V3 core, optional mode):** a read-only classical score/partition overlay view, opened via a visible button/tab toggle (never default-on).
- **Not in V3 core:** full notation editing or notation-first workflow.
...
- Partition view can be opened/closed in one tap from workbench/song-map using a visible control (e.g. `Counts | Partition`)
- Default remains count/timing view; partition is user-invoked and persists user/session preference
- Partition view playhead remains synchronized with section and timing landmarks
...
## Appendix A - Early Listener Synthesis (March 2026)

Source: `Review of early listeners of the pitch of ROAM` (early qualitative feedback)

Purpose: preserve signal from early reactions while keeping build decisions aligned with core PRD principles.

### Decision filter (applies to all incoming ideas)

Every suggestion is evaluated with this order:

1. Does it strengthen the primary user flow (choreographer/teacher creating work)?
2. Does it reduce in-studio friction during an active session?
3. Does it preserve authorship ("mirror, not muse")?
4. Is it required now (Part 0 / early) or later (post-core validation)?

If an idea fails #3, it does not enter near-term scope.

### Keep / Later / Reject matrix

| Feedback theme | Decision | Why |
|---|---|---|
| Better idea capture and recall for creators | **KEEP NOW** | Directly supports Part 0 and core problem (lost ideas, fragmented memory) |
| Prevent missed counts/phrase drift (e.g., missed 8-count) | **KEEP NOW** | Aligns with song-map/workbench timing clarity and rehearsal reliability |
| Stronger beat landmarks (drum/partition cues) | **KEEP NOW** | Improves timing navigation without changing product philosophy |
| Classical piano-style partition/staff view | **KEEP NOW (OPTIONAL MODE)** | Must ship in V3 as read-only toggle mode for score-literate users; default stays count/timing view |
| Dual-screen playback + recording on one device | **KEEP NOW (SCOPED OPTIONAL BETA)** | Implement in V3 behind explicit toggle only; fallback to standard capture when performance budgets are exceeded |
| Private practice support for enthusiasts | **LATER** | Valuable expansion, but secondary to creator-first workflow |
| Follow/compare learning mode for non-pro users | **LATER** | Possible growth path after core creator retention is proven |
| AI text-to-movement generation | **REJECT FOR NOW** | Conflicts with "mirror, not muse"; risks replacing authorship |
| Automated movement quality scoring ("angle good or not") | **REJECT FOR NOW** | Conflicts with non-judgment design principle |

### AI vision stance (explicit)

AI is a future option, not current product core.

- **Now:** build and validate creator workflow (capture -> structure -> memory -> assembly).
- **Later (only if validated):** introduce assistive AI that improves recall/organization, not authorship replacement.
- **Not in near-term scope:** generative choreography and evaluative scoring loops.

### Product implications for current roadmap

- Prioritize interaction and visual parity for `workbench`, `song-map`, `capture`, `library`, and `settings`.
- Continue improving timing and section clarity before adding expansion features.
- Keep future AI ideas in backlog as hypotheses gated by real retention and repeat project usage.
