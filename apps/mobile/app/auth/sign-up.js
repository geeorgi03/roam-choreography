...
function SignUpScreen() {
    ...
    if (configError || !supabase) {
        ...
        return (<react_native_1.View style={styles.container}>
        <react_native_1.View style={styles.content}>
          <react_native_1.Text style={styles.brandTitle}>{t('signUp.brandTitle')}</react_native_1.Text>
          ...
          <react_native_1.TouchableOpacity style={styles.link} onPress={() => expo_router_1.router.replace('/auth/sign-in')}>
            <react_native_1.Text style={styles.linkText}>{t('signUp.backToSignIn')}</react_native_1.Text>
          ...
          {__DEV__ && (<react_native_1.TouchableOpacity style={[styles.link, { marginTop: 16 }]} onPress={() => {
              ...
              <react_native_1.Text style={[styles.linkText, { color: theme_1.theme.light.mine }]}>{t('signUp.openAppDev')}</react_native_1.Text>
            </react_native_1.TouchableOpacity>)}
        </react_native_1.View>
      </react_native_1.View>);
    }
    return (<react_native_1.KeyboardAvoidingView style={styles.container} behavior={react_native_1.Platform.OS === 'ios' ? 'padding' : 'height'}>
      <react_native_1.View style={styles.content}>
        <react_native_1.Text style={styles.brandTitle}>{t('signUp.brandTitle')}</react_native_1.Text>
        <react_native_1.Text style={styles.tagline}>{t('signUp.tagline')}</react_native_1.Text>
        <react_native_1.View style={styles.form}>
          <react_native_1.Text style={styles.label}>{t('signUp.email')}</react_native_1.Text>
          <react_native_1.TextInput style={styles.input} placeholder={t('signUp.emailPlaceholder')} placeholderTextColor={theme_1.theme.light.muted} value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none" autoCorrect={false}/>
          <react_native_1.Text style={styles.label}>{t('signUp.password')}</react_native_1.Text>
          <react_native_1.TextInput style={styles.input} placeholder={t('signUp.passwordPlaceholder')} placeholderTextColor={theme_1.theme.light.muted} value={password} onChangeText={setPassword} secureTextEntry/>
          ...
          <react_native_1.TouchableOpacity style={[styles.button, loading && styles.buttonDisabled]} onPress={handleSignUp} disabled={loading || !supabase}>
            <react_native_1.Text style={styles.buttonText}>{loading ? t('signUp.creating') : t('signUp.createAccount')}</react_native_1.Text>
          </react_native_1.TouchableOpacity>
          <react_native_1.TouchableOpacity style={styles.link} onPress={() => expo_router_1.router.replace('/auth/sign-in')}>
            <react_native_1.Text style={styles.linkText}>{t('signUp.alreadyHaveAccount')}</react_native_1.Text>
          </react_native_1.TouchableOpacity>
        </react_native_1.View>
